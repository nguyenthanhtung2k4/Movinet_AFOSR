...
class AFORSVideoDataGenerator(Sequence):
    def __init__(self,
        ...
        self.classes = 12#np.unique(self.labels)
    ...
    @property
    def n_classes(self):
        return 12 #len(self.classes)

    
class AFORSVideoDataGeneratorMultistream(Sequence):
    def __init__(self,
                 video_dir,
                 annotation_file_path,
                 sampler=SystematicSampler(n_frames=16),
        ...
        self.clips = []
        self.labels = []
        self.rgb_video_dir = self.video_dir + "rgb"
        with open(self.annotation_file_path) as f:
            subject_dirs = [_.strip() for _ in f.readlines()]
            for subject_dir in subject_dirs:
                subject_dir_path = os.path.join(self.rgb_video_dir, subject_dir) #data/rgb/subject/ ; data/of/subject
    ...
    def __getitem__(self, index):
        ...
        for i in inds:
            #rgb
            video_file = os.path.join(self.rgb_video_dir, self.clips[i][0])
            frames = self.sampler(video_file, sample_id=i)
            if self.to_rgb:
                frames = [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) for frame in frames]
            if self.transform is not None:
            ...
            #optical flow
            of_video_file = video_file.replace("rgb","off")
            #print(of_video_file)
            frames = self.sampler(of_video_file, sample_id=i)
            if self.to_rgb:
                frames = [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) for frame in frames]
            if self.transform is not None:
                frames = [self.transform(frame) if not self.use_albumentations
                          else self.transform(image=frame)['image'] for frame in frames]
            of_X.append(np.stack(frames))
        ...
        if self.data_format == 'channels_first':
            X = X.transpose((0, 4, 1, 2, 3))
            of_X = of_X.transpose((0,4,1,2,3))
            
        y = np.array([self.labels[_] for _ in inds], dtype=np.int64)
        #print(X.shape)
        #print(of_X.shape)
        #XX = ([X, of_X])
        #XX  =  np.stack([X, of_X], axis=1)
        #YY = np.stack([y,y], axis=1)
        #print(len(XX))
        return [X, of_X], y
...
class AFORSVideoDataGeneratorFewShotLearning(Sequence):
    def __init__(self,
                 video_dir,
                 annotation_file_path,
                 list_class,
            ...
            for subject_dir in subject_dirs:
                ...
                for timestamp_dir in sorted(os.listdir(subject_dir_path)):
                    ...
                    for video_file in filter(lambda _: _.endswith('.mp4'),
                                             sorted(os.listdir(timestamp_dir_path))):
                        label = int(os.path.splitext(video_file)[0]) - 1
                        label = tf.one_hot(label,len(self.list_class))
                        video_file = os.path.join(subject_dir, timestamp_dir, video_file)
                        self.clips.append((video_file, subject_dir))
                        self.labels.append(label)
        self.classes = len(self.list_class) #np.unique(self.labels)
    ...
    @property
    def n_classes(self):
        return len(self.list_class) #len(self.classes)
