...
def main():
    ...
    logdir = f'/AFOSR/afosr2022/movinet/savedmodel/multistream/cross_env/Hieu/{tag}_{model_id}/{model_type}/{resolution}/logs/' + datetime.now().strftime("%Y%m%d-%H%M%S")
    checkpoint_filepath = f'/AFOSR/afosr2022/movinet/savedmodel/multistream/cross_env/Hieu/{tag}_{model_id}/{model_type}/{resolution}/checkpoint/cp.cpkt'
    savedmodel_filepath = f'/AFOSR/afosr2022/movinet/savedmodel/multistream/cross_env/Hieu/{tag}_{model_id}/{model_type}/{resolution}/model'
    # image transform
    transform = A.Compose([
    ...
    train_generator = AFORSVideoDataGeneratorMultistream(
        video_dir=video_dir,
    ...
    with strategy.scope():
        ...
        model_new = movinet_model.MovinetClassifier(
            backbone=backbone,
            num_classes=600)
        model_new.build([batch_size, temporal_slice, resolution, resolution, 3])
        ...
        model_weights = {w.name: w for w in model_new.weights}
        #print(model.weights)
        ...
        model_rgb = movinet_model.MovinetClassifier(
            backbone=backbone,
            num_classes=12)
        ...
        #model_rgb.layers[-1].trainable = True
        ...
        model_rgb.build([batch_size, temporal_slice, resolution, resolution, 3])
        out_rgb = model_rgb.output 
        
        #========Optical Flow=========================
        model_of = movinet_model.MovinetClassifier(
            backbone=backbone,
            num_classes=12)
        # Freeze all layers except for the final classifier head
        ...
        # the results from the gradient updates on the CPU
        #with tf.device("/cpu:0"):
        model_of.build([batch_size, temporal_slice, resolution, resolution, 3])
        out_of = model_of.output
        
        #=========Concatenation================================
        #out_rgb = tf.argmax(out_rgb, axis=1)
        #out_of = tf.argmax(out_of, axis=1)
        con = Concatenate()([out_rgb, out_of])
        ...
        # Output Layer
        out = Dense(12, activation='softmax')(out)

        model = Model(inputs=[model_rgb.input, model_of.input], outputs=[out])
    ...
    predict_result_filepath = f'/AFOSR/afosr2022/movinet/result/multistream/cross_env/Hieu/{tag}_{model_id}_{model_type}_{resolution}.csv'
    model_summary_filepath = f'/AFOSR/afosr2022/movinet/result/multistream/cross_env/Hieu/{tag}_{model_id}_{model_type}_{resolution}_flops.txt'
